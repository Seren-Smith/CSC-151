decksorted = [string(r) * s for s in "♣♦♥♠", r in "23456789TJQKA"]
lessthancard(card1, card2) = indexin(x, decksorted)[1] < indexin(y, decksorted)[1]
sortdeck(d) = sort(d, lt=lessthancard)
highestrank(d) = string(highestcard(d)[1])
 
# This will determine if we have duplicate cards. 
...
invalid(d) = !all(x -> x in decksorted, d) || hasduplicate(d)
...
# Assign a dictionary to various values.
...
# Functions to cover other various poker hands
...
# Sample hands
const hands =  [["2♥", "6♦", "2♣", "K♣", "Q♦"], ["2♥", "5♥", "7♦", "8♣", "9♠"], 
   ["A♥", "2♦", "3♣", "4♣", "5♦"], ["2♥", "3♥", "2♦", "3♣", "3♦"],
   ["2♥", "3♥", "2♦", "3♣", "3♦"], ["2♥", "7♥", "7♦", "9♣", "7♠"],
   ["T♥", "J♥", "Q♥", "K♥", "A♥"], ["4♥", "A♠", "K♠", "5♦", "T♠"],
   ["Q♣", "T♣", "7♣", "8♣", "4♣"]]
